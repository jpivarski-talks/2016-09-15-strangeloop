...
\vspace{0.75 cm}
\centering They all require all of the input data before giving you a histogram.
\vspace{0.75 cm}
\end{minipage}}
\end{center}
\vspace{5 cm}
...
\vspace{0.25 cm}
\uncover<2->{\fcolorbox{darkblue}{white}{\textcolor{darkblue}{Speedups range from nothing (string handling) to 150 times (sum, min, max, etc.).}}}
\vspace{0.25 cm}
\end{frame}
...
\item<4-> Data reduction (even a simple counter) is tricky on a GPU. Histogrammar generates a function that outputs JSON, to be plotted externally (Python, Scala, \ldots).
\end{itemize}
\end{frame}
